...
def run_with_bar(command):
    process = subprocess.Popen(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

    # Simulating progress for the sake of the example
    # In a real scenario, you would parse the output of the command to update progress
    for _ in tqdm(range(100), desc="Resizing filesystem", unit="%", ncols=100):
...
def resize_file_system():
    ...
    try:
        ...
        run_with_bar(f"sudo resize2fs {selected_partition} {new_size}")
...
def end_loop_devices():
    try:
        output = run_command(f" sudo losetup -D")
    except RuntimeError as e:
        print(e)
